...
// Best practice to export the function and reference it in the route.
    ...
    async function getAllEntries(_req: Request, res: Response) {
        try {
            ...
            if (!entities) sendResponseAsJson(res, 404, "No Entities found!", null);
            else {
                sendResponseAsJson(res, 200, "Success", entities);
            }
        } catch (error) {
            sendResponseAsJson(res, 500, `Failed to fetch ${entityName}s`, null);
            console.error(error);
        }
    }
    ...
    async function getSingleEntry(req: Request, res: Response) {
        try {
            ...
            if (!entity) sendResponseAsJson(res, 404, "No Entities found!", null);
            else {
                sendResponseAsJson(res, 200, "Success", entity);
            }
        } catch (error) {
            sendResponseAsJson(res, 500, `Failed to fetch ${entityName}`, null);
            console.error(error);
        }
    }
...
export function sendResponseAsJson(res: Response, code: number, message: string, entities: any) {
